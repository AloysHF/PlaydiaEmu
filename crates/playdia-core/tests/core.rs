...
use playdia_core::sh1::{CpuStop, Sh1};
use playdia_core::state::{crc32, decode_state, encode_state, ContentIdentity};

#[test]
fn bios_size_check() {
    assert!(matches!(load_bios(&[0u8; 10], false), Err(LoadError::BadBiosSize(10))));
...
fn machine_raw_disc_stream_counts() {
    // Craft a minimal raw disc with two form2 video sectors (file 0x61).
    let mut raw = vec![0u8; RAW_SECTOR * 4];
    for i in 0..4 {
        let off = i * RAW_SECTOR;
        raw[off + 15] = 2; // mode 2
        raw[off + 16] = 0x61; // file
        raw[off + 17] = 0; // channel
        raw[off + 18] = 0x64; // realtime + video-ish + form2? 0x20 form2 + 0x04 realtime
        raw[off + 19] = 0;
        raw[off + 20] = 0x61;
        raw[off + 21] = 0;
        raw[off + 22] = 0x64;
